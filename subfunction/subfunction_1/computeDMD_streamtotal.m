...
function [diag_dmd_evals,tdmd_evals]=computeDMD_stream(rpm,tsignal,xuhao,save_directory,name,T)
dt=T/length(rpm);
...
r = 5;      % maximum allowable rank of the DMD operator
...
S = S(1:r,1:r);
V = V(:,1:r);
Atilde = Qx' * X2 * V * inv(S);
[W,dmd_evals] = eig(Atilde);
